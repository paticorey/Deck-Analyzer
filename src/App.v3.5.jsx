...
export default function DeckAnalysis() {
  ...
  async function downloadRichAnalysisPdf() {
    ...
    const scatterChart = makeReportChart("scatter", "Price vs mana value", analysisCards.filter(c => Number(c.price) > 0 && typeof c.cmc === "number").sort((a, b) => Number(b.price) - Number(a.price)).slice(0, 30).map(c => ({ label: c.name, x: c.cmc || 0, y: Number(c.price) || 0 })), { width: 900, height: 400 });
    ...
    y += 238;
    addChartImage(scatterChart, M, y, pageW - M * 2, 250);
    y += 270;
    textBlock("The scatter plot highlights expensive/high-impact cards against mana value. Role and curve charts are based on the main deck only; sideboard and maybeboard are excluded.", M, pageW - M * 2, 9.5, color.muted);
    addFooter();
