...
function inferBoardFromSection(section, fallback = "main") {
  const lower = normalizeName(section || "");
  if (!lower) return fallback;
  if (lower.includes("sideboard") || lower === "side board" || lower === "sb" || lower.includes("side board")) return "sideboard";
  if (lower.includes("maybeboard") || lower.includes("maybe board") || lower.includes("considering") || lower.includes("maybe")) return "maybeboard";
  if (lower.includes("commander") || lower.includes("mainboard") || lower.includes("main deck") || lower === "main") return "main";
  return fallback;
}

function getCardBoard(card) {
  const explicit = normalizeName(card?.board || "");
  const tagText = [
    card?.category || "",
    card?.folder || "",
    card?.section || "",
    ...
    ...(Array.isArray(card?.manualTags) ? card.manualTags : []),
  ].join(" ");
  const raw = normalizeName(`${explicit} ${tagText}`).replace(/[()_\-]+/g, " ");
  if (explicit === "sideboard" || raw.includes("sideboard") || raw.includes("side board") || raw === "sb" || raw.includes(" sb ")) return "sideboard";
  if (explicit === "maybeboard" || raw.includes("maybeboard") || raw.includes("maybe board") || raw.includes("considering") || raw.includes("nodeck") || raw.includes("no deck") || raw.includes("no price")) return "maybeboard";
  return "main";
}

function isAnalysisCard(card) {
  return getCardBoard(card) === "main";
}

function getAnalysisCards(cards) {
  return (cards || []).filter(isAnalysisCard);
}
...
function normalizeParsedRows(rows) {
  ...
  for (const row of rows) {
    ...
    const board = row.board || inferBoardFromSection(row.category, "main");
    const storageKey = `${key}::${board}`;
    const existing = byName.get(storageKey);
...
function parseDeckCsv(text) {
  ...
  const categoryIdx = pickColumn(headers, ["category", "categories", "section", "folder", "categoria", "Categories", "board", "zone"]);
  const boardIdx = pickColumn(headers, ["board", "zone", "deck", "location"]);
  ...
  const rows = lines.slice(1).map(line => {
    ...
    return {
      ...
      board: boardIdx >= 0 ? inferBoardFromSection(row[boardIdx], inferBoardFromSection(categoryIdx >= 0 ? row[categoryIdx] : "", "main")) : inferBoardFromSection(categoryIdx >= 0 ? row[categoryIdx] : "", "main"),
  ...
  try {
    ...
    const rows = rawCards.map(x => {
      ...
      return {
        ...
        category: String(x.category || x.categories || x.section || x.board || x.zone || "").trim(),
        board: inferBoardFromSection(x.board || x.zone || x.category || x.categories || x.section || "", "main"),
...
function parseDecklist(text) {
  ...
  const headings = new Map([
    ...
    ["mainboard", "Other"], ["main deck", "Other"], ["sideboard", "Other"], ["side board", "Other"], ["maybeboard", "Other"], ["maybe board", "Other"],
  ]);
  ...
  for (const rawLine of lines) {
    ...
    if (line.startsWith("#")) {
      ...
      currentBoard = inferBoardFromSection(headingText, currentBoard);
      currentType = inferTypeFromSection(headingText, currentType);
      continue;
    }
    const headingKey = normalizeName(line.replaceAll(":", ""));
    if (headings.has(headingKey)) {
      currentBoard = inferBoardFromSection(headingKey, currentBoard);
      currentType = headings.get(headingKey);
      continue;
    }

    let lineBoard = currentBoard;
    if (line.toLowerCase().startsWith("sb:")) {
      lineBoard = "sideboard";
    ...
    rows.push({ qty, name, setCode: meta.setCode, collectorNumber: meta.collectorNumber, category: currentType, board: lineBoard, tags });
  }
...
const SCRYFALL_TOKEN_CACHE_PREFIX = "deckForgeAnalyzer.scryfallToken.v3.";

async function fetchScryfallTokenImageCached(tokenName) {
  const clean = cleanDisplayCardName(tokenName).replace(/ token$/i, "").trim();
  const key = normalizeName(clean);
  if (!key) throw new Error("empty token");
  const cacheKey = `${SCRYFALL_TOKEN_CACHE_PREFIX}${key}`;
  try {
    const raw = typeof window !== "undefined" ? window.localStorage.getItem(cacheKey) : null;
    if (raw) {
      const cached = JSON.parse(raw);
      if (cached?.image || cached?.smallImage) return cached;
    }
  } catch {}

  const queryAttempts = [
    `t:token name:${JSON.stringify(clean)}`,
    `t:token ${clean}`,
    `include:extras t:token name:${JSON.stringify(clean)}`,
    `include:extras t:token ${clean}`,
    `include:extras type:token oracle:${JSON.stringify(clean)}`,
  ];

  const scoreHit = (card) => {
    const n = normalizeName(card?.name || "").replace(/ token$/i, "").trim();
    const t = normalizeName(card?.type_line || "");
    const o = normalizeName(card?.oracle_text || "");
    let score = 0;
    if (t.includes("token")) score += 20;
    if (n === key) score += 50;
    if (n.includes(key) || key.includes(n)) score += 25;
    if (o.includes(key)) score += 8;
    if (card?.image_uris || card?.card_faces?.[0]?.image_uris) score += 10;
    return score;
  };

  for (const query of queryAttempts) {
    try {
      const res = await fetch(`https://api.scryfall.com/cards/search?unique=prints&include=extras&order=released&q=${encodeURIComponent(query)}`);
      if (!res.ok) continue;
      const data = await res.json();
      const hits = (data?.data || [])
        .filter(card => normalizeName(card?.type_line || "").includes("token"))
        .sort((a, b) => scoreHit(b) - scoreHit(a));
      const hit = hits[0];
      if (!hit || scoreHit(hit) < 25) continue;
      const img = imageFromScryfallData(hit);
      if (!img.image && !img.smallImage) continue;
      const payload = {
        name: normalizeTokenName(hit.name || clean, hit.type_line || "Token"),
        typeLine: hit.type_line || "Token",
        oracle: hit.oracle_text || "",
        image: img.image,
        smallImage: img.smallImage || img.image,
      };
      try { if (typeof window !== "undefined") window.localStorage.setItem(cacheKey, JSON.stringify(payload)); } catch {}
      return payload;
    } catch {}
  }

  // Last-resort friendly placeholders for common tokens. They still show as cards in the UI instead of empty boxes.
  const fallback = {
    name: clean,
    typeLine: `${clean} Token`,
    oracle: "Token image not found on Scryfall search.",
    image: "",
    smallImage: "",
    notFound: true,
  };
  try { if (typeof window !== "undefined") window.localStorage.setItem(cacheKey, JSON.stringify(fallback)); } catch {}
  throw new Error("token image not found");
}
...
function detectArchetypes(cards) {
  ...
  if (commanderHint?.mainTheme) {
    ...
    if (existingHintTheme) {
      existingHintTheme.rawScore = Math.round((existingHintTheme.rawScore || existingHintTheme.score || 0) + (commanderHint.weight || 0) * 0.35);
      existingHintTheme.score = existingHintTheme.rawScore;
  ...
  themeScores.sort((a, b) => (b.rawScore || b.score) - (a.rawScore || a.score) || a.name.localeCompare(b.name));
  const topRawThemeScore = Math.max(1, themeScores[0]?.rawScore || themeScores[0]?.score || 1);
  themeScores.forEach((theme, index) => {
    const raw = theme.rawScore || theme.score || 0;
    const ratio = Math.max(0, Math.min(1, raw / topRawThemeScore));
    // Theme score is a confidence-style index, not a raw count.
    // Use a curved relative score so secondary themes do not all sit at 100 when many roles overlap.
    const curved = Math.pow(ratio, 1.65);
    const normalized = Math.round(18 + curved * 78 - index * 7);
    const cap = index === 0 ? 96 : 84 - Math.min(18, index * 3);
    theme.score = Math.max(8, Math.min(cap, normalized));
  });
  if (themeScores[0]) themeScores[0].score = Math.max(themeScores[0].score, 82);
  themeScores.sort((a, b) => b.score - a.score || (b.rawScore || 0) - (a.rawScore || 0) || a.name.localeCompare(b.name));
...
async function computeSpellbookCombosFromVariants(variants, cards) {
  const deckNames = new Set(cards.map(c => normalizeName(c.name)));
  const includedCandidates = [];
  const almostCandidates = [];

  // Performance note: Commander Spellbook bulk can contain a very large number of variants.
  // We first do a cheap overlap check against the deck before fully normalizing a combo.
  // This keeps the full information available, but avoids processing thousands of irrelevant lines.
  for (let i = 0; i < variants.length; i++) {
    const raw = variants[i];
    const pieces = [...new Set(extractCsbCards(raw))];
    if (pieces.length < 2 || pieces.length > 4) continue;

    const presentCount = pieces.reduce((acc, piece) => acc + (deckNames.has(normalizeName(piece)) ? 1 : 0), 0);
    const isComplete = presentCount === pieces.length;
    const isAlmost = presentCount === pieces.length - 1;
    if (!isComplete && !isAlmost) continue;

    const combo = normalizeSpellbookCombo(raw, cards);
    if (combo.pieces.length < 2 || combo.pieces.length > 4) continue;
    if (combo.complete) includedCandidates.push({ ...combo, complete: true, source: "Commander Spellbook Bulk" });
    else if (combo.missing.length === 1 && combo.present.length === combo.pieces.length - 1) almostCandidates.push({ ...combo, complete: false, source: "Commander Spellbook Bulk" });

    // Yield back to the browser so the UI does not freeze during a large bulk scan.
    if (i > 0 && i % 750 === 0) await sleep(0);
  }

  const included = dedupeCombos(includedCandidates)
    .sort((a, b) => a.pieces.length - b.pieces.length || a.name.localeCompare(b.name));

  const almost = dedupeCombos(almostCandidates)
...
async function fetchSpellbookBulkCombos(cards) {
  ...
  for (const url of urls) {
    try {
      ...
      return await computeSpellbookCombosFromVariants(variants, cards);
    } catch (err) {
      lastError = err;
    }
  }
...
async function fetchCommanderSpellbookCombosForCards(cards) {
  // Fast path first: the API is lighter when available. If it fails or returns unusable data,
  // the bulk database remains available as a full fallback so we do not lose combo information.
  try {
    const apiResult = await fetchCommanderSpellbookApiCombos(cards);
    if ((apiResult.included?.length || 0) || (apiResult.almost?.length || 0)) return apiResult;
  } catch (apiError) {
    // fallback below
  }
  ...
  try {
    ...
    throw new Error(`Spellbook unavailable: ${bulkError?.message || "bulk"}`);
  }
}
...
export default function DeckAnalysis() {
  ...
  const [tokenImageData, setTokenImageData] = useState({});
  ...
  const analysisCards = useMemo(() => getAnalysisCards(cards), [cards]);
  const sideboardCards = useMemo(() => cards.filter(c => getCardBoard(c) !== "main"), [cards]);
  const commander = useMemo(() => getCommander(analysisCards), [analysisCards]);
  const deckAllowedColors = useMemo(() => getDeckAllowedColors(analysisCards), [analysisCards]);
  const totalDeckPrice = useMemo(() => getDeckTotalPrice(analysisCards), [analysisCards]);
  const totalCards = useMemo(() => analysisCards.reduce((a, c) => a + c.qty, 0), [analysisCards]);
  const sideboardTotalCards = useMemo(() => sideboardCards.reduce((a, c) => a + c.qty, 0), [sideboardCards]);
  const pricedCardsCount = useMemo(() => analysisCards.filter(c => Number(c.price) > 0).reduce((a, c) => a + c.qty, 0), [analysisCards]);
  const unpricedCardsCount = useMemo(() => analysisCards.filter(c => !(Number(c.price) > 0)).reduce((a, c) => a + c.qty, 0), [analysisCards]);
  const landsCount = useMemo(() => analysisCards.filter(c => c.type === "Lands").reduce((a, c) => a + c.qty, 0), [analysisCards]);
  const manaCurve = useMemo(() => calculateManaCurve(analysisCards), [analysisCards]);
  const cardTypes = useMemo(() => countBy(analysisCards, c => c.type).map(d => ({ ...d, color: CARD_TYPE_COLORS[d.name] || CARD_TYPE_COLORS.Other })), [analysisCards]);
  const roles = useMemo(() => aggregateRoles(analysisCards), [analysisCards]);
  const scores = useMemo(() => calculateScores(analysisCards), [analysisCards]);
  const bracket = useMemo(() => calculateBracket(analysisCards), [analysisCards]);
  const archetypes = useMemo(() => detectArchetypes(analysisCards), [analysisCards]);
  const combos = useMemo(() => detectCombos(analysisCards), [analysisCards]);
  const recommendations = useMemo(() => suggestRecommendations(analysisCards), [analysisCards]);
  const avgCMC = useMemo(() => averageCmc(analysisCards), [analysisCards]);
  const planText = useMemo(() => buildPlanText(analysisCards), [analysisCards]);
  const gamePhases = useMemo(() => buildGamePhases(analysisCards), [analysisCards]);
  const unknownCmc = useMemo(() => unknownCmcCount(analysisCards), [analysisCards]);
  ...
  const autoAddRecommendations = useMemo(() => {
    const owned = new Set(analysisCards.map(c => normalizeName(c.name)));
    const out = [];
  ...
  }, [analysisCards, recommendations, almostCombos]);
  ...
  const synergyLines = useMemo(() => detectSynergyLines(analysisCards), [analysisCards]);
  const strengths = useMemo(() => generateStrengths(analysisCards), [analysisCards]);
  const weaknesses = useMemo(() => generateWeaknesses(analysisCards), [analysisCards]);
  const cutCandidates = useMemo(() => getCutCandidates(analysisCards, 10), [analysisCards]);
  const manaStats = useMemo(() => getManaStats(), [analysisCards]);
  const colorCurveGroups = useMemo(() => getColorCurveGroups(), [analysisCards]);
  const tokensAndExtras = useMemo(() => tokensLoaded ? detectTokensAndExtras() : { tokens: [], extras: [] }, [analysisCards, tokensLoaded]);
  ...
  const allCmcBuckets = useMemo(() => ["All", "0", "1", "2", "3", "4", "5", "6", "7+", "Unknown"], []);
  const allColorFilters = useMemo(() => ["All", "W", "U", "B", "R", "G", "Colorless", "Multicolor"], []);
  const allBoardFilters = useMemo(() => ["All", "Main deck", "Sideboard", "Maybeboard"], []);
  const filteredCards = useMemo(() => {
    ...
    if (colorFilter !== "All") {
      base = base.filter(c => {
        ...
        if (colorFilter === "Colorless") return ci.length === 0;
        if (colorFilter === "Multicolor") return ci.length > 1;
        return ci.includes(colorFilter);
      });
    }
    if (cmcFilter !== "All") {
      base = base.filter(c => {
        const bucket = getCmcBucket(c);
        return cmcFilter === "Unknown" ? !bucket : bucket === cmcFilter;
      });
    }
    if (boardFilter !== "All") {
      const boardValue = boardFilter === "Main deck" ? "main" : boardFilter === "Sideboard" ? "sideboard" : "maybeboard";
      base = base.filter(c => getCardBoard(c) === boardValue);
    }
    if (priceFilter !== "All") {
      base = base.filter(c => {
        const price = Number(c.price) || 0;
        if (priceFilter === "Priced") return price > 0;
        if (priceFilter === "No price") return price <= 0;
        if (priceFilter === "Budget <= 1") return price > 0 && price <= 1;
        if (priceFilter === "1 - 5") return price > 1 && price <= 5;
        if (priceFilter === "5 - 10") return price > 5 && price <= 10;
        if (priceFilter === "Premium > 10") return price > 10;
        return true;
      });
    }
    if (cardSearch.trim()) {
      const q = normalizeName(cardSearch);
      base = base.filter(c => normalizeName(`${c.name} ${c.typeLine || ""} ${getEffectiveRoles(c).join(" ")} ${boardDisplayName(getCardBoard(c))}`).includes(q));
    }
    const sorted = [...base].sort((a, b) => {
      if (cardSortBy === "mv") return (a.cmc ?? 99) - (b.cmc ?? 99) || a.name.localeCompare(b.name);
      if (cardSortBy === "price-desc") return (Number(b.price) || 0) - (Number(a.price) || 0) || a.name.localeCompare(b.name);
      if (cardSortBy === "price-asc") return (Number(a.price) || 0) - (Number(b.price) || 0) || a.name.localeCompare(b.name);
      if (cardSortBy === "qty") return (b.qty || 0) - (a.qty || 0) || a.name.localeCompare(b.name);
      if (cardSortBy === "type") return String(a.type || "").localeCompare(String(b.type || "")) || a.name.localeCompare(b.name);
      return a.name.localeCompare(b.name);
    });
    return sorted;
  }, [cards, activeFilter, cardSearch, typeFilter, roleFilter, colorFilter, cmcFilter, boardFilter, priceFilter, cardSortBy]);
  ...
  }, [cardSearch, typeFilter, roleFilter, colorFilter, cmcFilter, boardFilter, priceFilter, cardSortBy, activeFilter]);
  ...
  useEffect(() => {
    ...
    async function loadTokenImages() {
      const missingTokens = (tokensAndExtras.tokens || [])
        .filter(token => !(token.image || token.smallImage || tokenImageData[normalizeName(token.name)]?.image || tokenImageData[normalizeName(token.name)]?.smallImage))
        .map(token => token.name)
        .filter(Boolean)
        .filter(name => !tokenImageData[normalizeName(name)])
        .slice(0, 16);
      for (const name of missingTokens) {
        const key = normalizeName(name);
        try {
          const payload = await fetchScryfallTokenImageCached(name);
          if (!cancelled) setTokenImageData(prev => ({ ...prev, [key]: payload }));
        } catch {
          if (!cancelled) setTokenImageData(prev => ({ ...prev, [key]: { name, image: "", smallImage: "", typeLine: "Token", notFound: true } }));
        }
        await sleep(25);
      }
    }
    if (tokensLoaded && tokensAndExtras.tokens?.length) loadTokenImages();
    return () => { cancelled = true; };
  }, [tokensLoaded, tokensAndExtras.tokens]);
  ...
  async function analyzeDeck() {
    ...
    setCards(parsed);
    setTab("overview");
    await enrichCardsList(parsed);
    setSpellbookStatus("Local combo scan ready. Click Load Commander Spellbook combos for the full external database.");
  }
  ...
  function loadSavedDeck(deck) {
    ...
    setCards((deck.cards || []).map(c => ({ ...c, board: c.board || "main", roles: detectRoles(c) })));
    setActiveFilter(null);
    setCardSearch("");
    setCardVisibleCount(40);
    setTypeFilter("All");
  ...
  function loadJsPdfLibrary() {
    ...
    if (window.__deckForgeJsPdfPromise) return window.__deckForgeJsPdfPromise;
    window.__deckForgeJsPdfPromise = new Promise((resolve, reject) => {
      const existing = document.querySelector('script[data-deckforge-jspdf="true"]');
      if (existing) {
        existing.addEventListener("load", () => window.jspdf?.jsPDF ? resolve(window.jspdf.jsPDF) : reject(new Error("jsPDF missing")), { once: true });
      ...
      script.async = true;
      script.dataset.deckforgeJspdf = "true";
    ...
    return window.__deckForgeJsPdfPromise;
  }

  async function loadImageDataUrl(url) {
    if (!url || typeof window === "undefined") return null;
    // Fetching the image as a blob is more reliable than drawing a cross-origin image to canvas.
    try {
    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        try {
          ...
          const maxW = 360;
          const ratio = img.naturalWidth ? img.naturalHeight / img.naturalWidth : 1.4;
          canvas.width = maxW;
  ...
  function makeReportChart(kind, title, data, options = {}) {
    ...
    const palette = options.palette || ["#16a34a", "#0ea5e9", "#f59e0b", "#a855f7", "#ef4444", "#14b8a6", "#84cc16", "#64748b", "#22c55e"];
    const rows = (data || []).filter(x => Number(x.value ?? x.count ?? x.y) > 0).slice(0, options.limit || 9);
    ctx.fillStyle = "#ffffff";
    ...
    if (!rows.length) {
      ...
      ctx.fillText("No data available", 36, 98);
      return canvas.toDataURL("image/png");
    }
    if (kind === "donut") {
      const total = rows.reduce((a, r) => a + Number(r.value ?? r.count ?? 0), 0) || 1;
      let start = -Math.PI / 2;
      const cx = W * 0.28;
      const cy = H * 0.55;
      ...
      rows.forEach((row, i) => {
        const y0 = 102 + i * 34;
        ctx.fillStyle = palette[i % palette.length];
        ctx.fillRect(W * 0.56, y0 - 14, 18, 18);
        ctx.fillStyle = "#0f172a";
        ctx.font = "800 17px Arial";
        ctx.fillText(String(row.label || row.name || row.role || row.subject).slice(0, 30), W * 0.56 + 28, y0);
        ctx.fillStyle = "#475569";
        ctx.font = "800 15px Arial";
        ctx.fillText(String(Math.round(Number(row.value ?? row.count ?? 0))), W - 92, y0);
      });
      return canvas.toDataURL("image/png");
    }
    if (kind === "valueList") {
      const totalValue = Number(options.totalValue || rows.reduce((a, r) => a + Number(r.value ?? r.price ?? 0), 0)) || 1;
      const maxValue = Math.max(1, ...rows.map(r => Number(r.value ?? r.price ?? 0)));
      const left = 42;
      const top = 92;
      const rowH = Math.min(56, Math.max(44, (H - top - 38) / Math.max(1, rows.length)));
      const barX = W * 0.50;
      const barW = W - barX - 118;
      rows.slice(0, 5).forEach((row, i) => {
        const y0 = top + i * rowH;
        const value = Number(row.value ?? row.price ?? 0);
        const share = totalValue ? (value / totalValue) * 100 : 0;
        const label = String(row.label || row.name || "Card");
        ctx.fillStyle = i === 0 ? "#fef3c7" : "#f8fafc";
        ctx.beginPath();
        if (ctx.roundRect) ctx.roundRect(left, y0 - 24, W - left * 2, rowH - 8, 14);
        else ctx.rect(left, y0 - 24, W - left * 2, rowH - 8);
        ctx.fill();
        ctx.fillStyle = palette[i % palette.length];
        ctx.beginPath();
        ctx.arc(left + 22, y0 - 4, 14, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#ffffff";
        ctx.font = "900 17px Arial";
        ctx.textAlign = "center";
        ctx.fillText(String(i + 1), left + 22, y0 + 2);
        ctx.textAlign = "left";
        ctx.fillStyle = "#0f172a";
        ctx.font = "900 18px Arial";
        ctx.fillText(label.slice(0, 34), left + 48, y0 - 6);
        ctx.fillStyle = "#64748b";
        ctx.font = "700 13px Arial";
        ctx.fillText(`${share.toFixed(1)}% of deck value`, left + 48, y0 + 13);
        ctx.fillStyle = "#dcfce7";
        ctx.beginPath();
        if (ctx.roundRect) ctx.roundRect(barX, y0 - 14, barW, 12, 6);
        else ctx.rect(barX, y0 - 14, barW, 12);
        ctx.fill();
        ctx.fillStyle = palette[i % palette.length];
        ctx.beginPath();
        if (ctx.roundRect) ctx.roundRect(barX, y0 - 14, Math.max(8, (value / maxValue) * barW), 12, 6);
        else ctx.rect(barX, y0 - 14, Math.max(8, (value / maxValue) * barW), 12);
        ctx.fill();
        ctx.fillStyle = "#14532d";
        ctx.font = "900 17px Arial";
        ctx.textAlign = "right";
        ctx.fillText(`${value.toFixed(2)} EUR`, W - left - 10, y0 - 6);
        ctx.textAlign = "left";
      });
      return canvas.toDataURL("image/png");
    }
    if (kind === "scatter") {
      const points = rows.map(r => ({ x: Number(r.x ?? r.cmc ?? 0), y: Number(r.y ?? r.price ?? 0), label: r.label || r.name || "card" }));
      const maxX = Math.max(1, ...points.map(p => p.x));
      const maxY = Math.max(1, ...points.map(p => p.y));
      const x0 = 86, y0 = H - 70, plotW = W - 150, plotH = H - 150;
      ...
      ctx.fillStyle = "#64748b";
      ctx.font = "800 15px Arial";
      ctx.fillText("Mana value", x0 + plotW - 94, y0 + 34);
      ctx.save();
      ctx.translate(28, y0 - plotH / 2 + 40);
      ctx.rotate(-Math.PI / 2);
      ctx.fillText("Price", 0, 0);
      ctx.restore();
      points.forEach((pt, i) => {
        const px = x0 + (pt.x / maxX) * plotW;
        const py = y0 - (pt.y / maxY) * plotH;
        ctx.fillStyle = palette[i % palette.length];
    ...
    const max = Math.max(1, ...rows.map(x => Number(x.value ?? x.count ?? 0)));
    const left = 42;
    const labelW = options.compact ? 150 : 190;
    const rightPad = 88;
    const top = 86;
    const rowH = Math.min(42, Math.max(26, (H - top - 38) / rows.length));
    const barW = W - left - labelW - rightPad;
    rows.forEach((row, i) => {
      const y0 = top + i * rowH;
      const label = String(row.label || row.name || row.subject || row.role || "Data").slice(0, options.compact ? 22 : 30);
      const value = Number(row.value ?? row.count ?? 0);
      ctx.fillStyle = "#0f172a";
      ctx.font = "800 16px Arial";
      ctx.fillText(label, left, y0 + 15);
      ctx.fillStyle = "#dcfce7";
      ctx.beginPath();
      if (ctx.roundRect) {
        ctx.roundRect(left + labelW, y0, barW, 18, 9);
        ctx.fill();
      } else {
        ctx.fillRect(left + labelW, y0, barW, 18);
      }
      ctx.fillStyle = palette[i % palette.length];
      const filled = Math.max(4, (value / max) * barW);
      ctx.beginPath();
      if (ctx.roundRect) {
        ctx.roundRect(left + labelW, y0, filled, 18, 9);
        ctx.fill();
      } else {
        ctx.fillRect(left + labelW, y0, filled, 18);
      }
      ctx.fillStyle = "#0f172a";
      ctx.font = "900 15px Arial";
      ctx.fillText(String(Math.round(value)), left + labelW + barW + 16, y0 + 15);
    });
    return canvas.toDataURL("image/png");
  }

  async function downloadRichAnalysisPdf() {
    ...
    const setText = (rgb = color.ink) => doc.setTextColor(...rgb);
    const imageFormat = (dataUrl) => String(dataUrl || "").includes("image/png") ? "PNG" : "JPEG";
    const safeAddImage = (dataUrl, x, yy, w, h) => {
      if (!dataUrl) return false;
      try { doc.addImage(dataUrl, imageFormat(dataUrl), x, yy, w, h); return true; } catch { return false; }
    };
    const addFooter = () => {
      doc.setFont("helvetica", "normal");
      doc.setFontSize(8);
      doc.setTextColor(100, 116, 139);
      doc.text(`DeckForge Analyzer ${APP_VERSION} - Unofficial fan project`, M, pageH - 24);
      doc.text(`Page ${doc.internal.getNumberOfPages()}`, pageW - M - 38, pageH - 24);
    };
    const newPage = () => { addFooter(); doc.addPage(); y = 42; };
    const need = (height) => { if (y + height > pageH - 58) newPage(); };
    const textBlock = (txt, x, maxW, size = 10, rgb = color.ink, bold = false, lineGap = 1.25) => {
      const clean = pdfCleanText(txt);
      if (!clean) return;
      doc.setFont("helvetica", bold ? "bold" : "normal");
      doc.setFontSize(size);
      setText(rgb);
      const lines = doc.splitTextToSize(clean, maxW);
      need(lines.length * size * lineGap + 10);
      doc.text(lines, x, y);
      y += lines.length * size * lineGap + 7;
    };
    const fixedText = (txt, x, yy, maxW, size = 10, rgb = color.ink, bold = false, lineGap = 1.18) => {
      doc.setFont("helvetica", bold ? "bold" : "normal");
      doc.setFontSize(size);
      setText(rgb);
      const lines = doc.splitTextToSize(pdfCleanText(txt), maxW);
      doc.text(lines, x, yy);
      return yy + lines.length * size * lineGap;
    };
    const sectionTitle = (title) => {
      ...
      doc.setFontSize(18);
      setText(color.green);
      doc.text(pdfCleanText(title), M, y);
      y += 18;
    };
    const chip = (label, x, yy, fill = [220, 252, 231], rgb = color.green, maxW = 138) => {
      const text = pdfCleanText(label);
      const w = Math.min(maxW, Math.max(56, 16 + text.length * 4.9));
      doc.setFillColor(...fill);
      doc.roundedRect(x, yy - 13, w, 20, 9, 9, "F");
      doc.setFont("helvetica", "bold");
      doc.setFontSize(8);
      setText(rgb);
      doc.text(text.slice(0, 28), x + 8, yy);
      return w;
    };
    const bullet = (txt, indent = 10) => textBlock(`- ${txt}`, M + indent, pageW - M * 2 - indent, 9.5, color.ink, false, 1.28);
    const addChartImage = (img, x, yy, w, h) => {
      if (!img) return;
      doc.setDrawColor(...color.line);
      doc.setLineWidth(1);
      doc.roundedRect(x, yy, w, h, 8, 8, "S");
      safeAddImage(img, x + 3, yy + 3, w - 6, h - 6);
    };
    const roleChart = makeReportChart("bar", "Role diagnosis", scores.radar.map(x => ({ label: x.subject, value: x.value })), { width: 900, height: 420, limit: 8, compact: true });
    const roleDonut = makeReportChart("donut", "Role mix", roles.slice(0, 8).map(x => ({ label: x.role, value: x.count })), { width: 900, height: 420 });
    const curveChart = makeReportChart("bar", "Mana curve", manaCurve.map(x => ({ label: `MV ${x.cmc}`, value: x.count })), { width: 900, height: 400, limit: 8 });
    const topValueChart = makeReportChart("valueList", "Top 5 most expensive cards", analysisCards
      .filter(c => Number(c.price) > 0)
      .sort((a, b) => Number(b.price) - Number(a.price))
      .slice(0, 5)
      .map(c => ({ label: c.name, value: Number(c.price) * (c.qty || 1) })),
      { width: 900, height: 400, totalValue: totalDeckPrice });

    doc.setFillColor(248, 250, 252);
    doc.rect(0, 0, pageW, pageH, "F");

    // Cover / identity card. Keep the header spacious so long commander names never overlap the KPI chips.
    const coverY = y;
    const coverH = 152;
    doc.setFillColor(...color.soft);
    doc.roundedRect(M, coverY, pageW - M * 2, coverH, 18, 18, "F");
    doc.setDrawColor(...color.line);
    doc.roundedRect(M, coverY, pageW - M * 2, coverH, 18, 18, "S");

    let commanderImgUrl = commander?.image || commander?.smallImage || "";
    let commanderData = await loadImageDataUrl(commanderImgUrl);
    if (!commanderData && (commander?.name || deckName)) {
      try {
        const rawCommander = await fetchScryfallCardCached(commander?.name || deckName);
        const scryCommander = scryToCardData(rawCommander);
        commanderImgUrl = scryCommander.image || scryCommander.smallImage || commanderImgUrl;
        commanderData = await loadImageDataUrl(commanderImgUrl);
      } catch {}
    }
    if (commanderData && safeAddImage(commanderData, M + 18, coverY + 16, 86, 120)) {
      doc.setDrawColor(20, 83, 45);
      doc.roundedRect(M + 18, coverY + 16, 86, 120, 7, 7, "S");
    } else {
      doc.setFillColor(...color.green);
      doc.roundedRect(M + 18, coverY + 16, 86, 120, 8, 8, "F");
      doc.setFont("helvetica", "bold");
      doc.setFontSize(11);
      doc.setTextColor(220, 252, 231);
      doc.text("Commander", M + 29, coverY + 80);
    }

    const titleX = M + 122;
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10.5);
    setText(color.bright);
    doc.text("COMMANDER DECK ANALYSIS", titleX, coverY + 30);
    const titleBottomY = fixedText(commander?.name || deckName || "Commander not detected", titleX, coverY + 58, pageW - titleX - M - 8, 23, color.ink, true, 1.05);
    const subtitle = `${archetypes[0]?.macroArchetype || "Deck"} • ${archetypes[0]?.mainTheme || archetypes[0]?.name || "Strategy"}`;
    fixedText(subtitle, titleX, Math.min(coverY + 104, titleBottomY + 12), pageW - titleX - M - 8, 9.5, color.muted, false, 1.15);
    const chipY = coverY + coverH - 24;
    let chipX = titleX;
    chipX += chip(`Overall ${scores.overall}/10`, chipX, chipY, [254, 243, 199], color.gold, 112) + 8;
    chipX += chip(`Bracket ${bracket.bracket} - ${bracket.shortLabel}`, chipX, chipY, [220, 252, 231], color.green, 150) + 8;
    chip(`${formatMoney(totalDeckPrice)}`, chipX, chipY, [219, 234, 254], color.blue, 112);
    y = coverY + coverH + 34;

    sectionTitle("Executive summary");
    const summary = [
      `Main deck analyzed: ${totalCards} cards. Lands: ${landsCount}. Average mana value: ${avgCMC}.`,
      ...
      sideboardTotalCards ? `Sideboard/maybeboard kept separate: ${sideboardTotalCards} card(s), not counted in scores, bracket, combos or charts.` : "No sideboard/maybeboard cards detected.",
    ];
    summary.forEach(item => bullet(item));

    sectionTitle("Visual snapshot");
    const halfW = (pageW - M * 2 - 16) / 2;
    addChartImage(roleChart, M, y, halfW, 182);
    addChartImage(curveChart, M + halfW + 16, y, halfW, 182);
    y += 198;
    textBlock("Role diagnosis and mana curve use the main deck only; sideboard and maybeboard are excluded.", M, pageW - M * 2, 8.8, color.muted);
    addFooter();

    doc.addPage();
    y = 42;
    sectionTitle("Value and role mix");
    addChartImage(roleDonut, M, y, halfW, 218);
    addChartImage(topValueChart, M + halfW + 16, y, halfW, 218);
    y += 236;
    textBlock("Top value cards are ordered by estimated total price. Missing prices are excluded from the value ranking.", M, pageW - M * 2, 8.8, color.muted);

    sectionTitle("Archetype explained");
    textBlock(`Macroarchetype: ${archetypes[0]?.macroArchetype || "Not detected"}`, M, pageW - M * 2, 11.5, color.ink, true);
    textBlock(`Main theme: ${archetypes[0]?.mainTheme || "Not detected"} - confidence ${archetypes[0]?.confidence || archetypes[0]?.score || "?"}%`, M, pageW - M * 2, 11);
    (archetypes[0]?.explanation || []).slice(0, 5).forEach(line => bullet(line));
    if (archetypes[0]?.evidenceCards?.length || archetypes[0]?.evidence?.length) bullet(`Evidence cards: ${(archetypes[0]?.evidenceCards || archetypes[0]?.evidence || []).slice(0, 10).join(", ")}`);

    sectionTitle("Strengths & weaknesses");
    doc.setFillColor(240, 253, 244);
    doc.roundedRect(M, y, pageW - M * 2, 28, 8, 8, "F");
    textBlock("Strengths", M + 10, pageW - M * 2 - 20, 12, color.green, true);
    strengths.slice(0, 5).forEach(x => bullet(`${x.title}: ${x.text}`));
    y += 4;
    doc.setFillColor(254, 242, 242);
    doc.roundedRect(M, y, pageW - M * 2, 28, 8, 8, "F");
    textBlock("Weaknesses", M + 10, pageW - M * 2 - 20, 12, color.red, true);
    weaknesses.slice(0, 5).forEach(x => bullet(`${x.title}: ${x.text}`));
    addFooter();

    doc.addPage();
    y = 42;
    sectionTitle("Combos, upgrades and cuts");
    textBlock("Included combos", M, pageW - M * 2, 12, color.green, true);
    (includedCombos.length ? includedCombos.slice(0, 7) : [{ name: "No complete combos detected", effects: [] }]).forEach(x => bullet(`${x.name}${x.effects?.length ? `: ${x.effects.join(" / ")}` : ""}`));
    textBlock("Almost included", M, pageW - M * 2, 12, color.gold, true);
    almostCombos.slice(0, 7).forEach(x => bullet(`${x.name}: missing ${(x.missing || []).join(", ")}`));
    textBlock("Recommended upgrades", M, pageW - M * 2, 12, color.blue, true);
    recommendations.slice(0, 7).forEach(x => bullet(`${x.area}: ${x.card} (${x.priority})`));
    textBlock("Possible cuts", M, pageW - M * 2, 12, color.red, true);
    cutCandidates.slice(0, 7).forEach(x => bullet(`${x.card.name}: ${(x.reasons || []).slice(0, 2).join(" / ") || "reviewable slot"}`));
    sectionTitle("Showcase summary");
    textBlock("This report intentionally omits the full decklist so it stays clean and easy to share. Use the app's Deck and Cards tabs for the full list, prices and alternate art choices.", M, pageW - M * 2, 10, color.muted);
    textBlock(`Best role packages: ${roles.slice(0, 5).map(r => `${r.role} (${r.count})`).join(", ") || "none detected"}.`, M, pageW - M * 2, 10);
    addFooter();
    doc.save(`${sanitizeFilename(commander?.name || deckName || "deck")}_DeckForge_${APP_VERSION}.pdf`);
  }
  ...
  async function downloadAnalysisPdf() {
    ...
    setReportStatus("Building PDF analysis...");
    try {
      await downloadRichAnalysisPdf();
      setReportStatus("Analysis PDF downloaded.");
    } catch (err) {
      console.warn("Rich PDF failed, using fallback PDF", err);
      downloadSimpleAnalysisPdf();
    }
  }

  function downloadSimpleAnalysisPdf() {
    if (!cards.length) {
      setReportStatus("Analyze a decklist first to download the analysis.");
      return;
    }
    ...
    bulletList([
      ...
      `Bracket: ${bracket.bracket} (${bracket.shortLabel})`,
      `Estimated price: ${formatMoney(totalDeckPrice)} (${unpricedCardsCount} cards without price detected)`,
      `Main deck cards analyzed: ${totalCards} - Lands: ${landsCount} - Average CMC: ${avgCMC}`,
      sideboardTotalCards ? `Sideboard/maybeboard: ${sideboardTotalCards} card(s), excluded from scores and charts` : "No sideboard/maybeboard cards detected",
      `Archetype: ${archetypes[0]?.macroArchetype || "Not detected"} - ${archetypes[0]?.mainTheme || archetypes[0]?.name || "Not detected"}`,
      `Subplans: ${(archetypes[0]?.subThemes || []).join(" / ") || "Not detected"}`,
    ]);
    ...
    bulletList(cutCandidates.slice(0, 10).map(x => `Possible cut: ${x.card.name} - ${(x.reasons || []).slice(0, 2).join(" / ") || "reviewable slot"}`), 10);

    section("Showcase summary");
    wrapped("This PDF intentionally omits the full decklist so the report stays clean and shareable. Use the app's Cards and Deck tabs for the full list and alternate art choices.", M, W - M * 2, 10, "F1", "#475569", 14);
    bulletList([
      `Main deck analyzed: ${totalCards} cards`,
      sideboardTotalCards ? `Sideboard/maybeboard kept separate: ${sideboardTotalCards} cards not counted in scores` : "No sideboard cards detected",
      `Cards with price: ${pricedCardsCount}; unpriced cards: ${unpricedCardsCount}`,
      `Best role packages: ${roles.slice(0, 5).map(r => `${r.role} (${r.count})`).join(", ") || "none detected"}`,
      `Top evidence cards: ${(archetypes[0]?.evidenceCards || archetypes[0]?.evidence || []).slice(0, 8).join(", ") || "not detected"}`
    ], 8);
  ...
  function getDeckViewGroup(card) {
    const board = getCardBoard(card);
    if (board !== "main") return boardDisplayName(board);
  ...
  async function enrichScryfall() {
    ...
    if (spellbookHasFetched) await refreshSpellbookCombos(getAnalysisCards(enriched));
    else setSpellbookStatus("Scryfall refreshed. Click Load Commander Spellbook combos when you want the full combo database.");
  }

  async function refreshSpellbookCombos(inputCards = analysisCards) {
  ...
  function getDiagnosticDetail(subject) {
    ...
    const details = {
      "Manabase": {
        ...
        cards: analysisCards.filter(c => c.type === "Lands").slice(0, 12).map(c => `${c.qty}x ${c.name}`),
      },
      "Ramp": {
        title: "Ramp",
  ...
  const statCards = [
    { label: "Cards", value: cards.length ? String(totalCards) : "-", icon: "🃏", color: "#4ade80", sub: totalCards === 100 ? `Commander legal${sideboardTotalCards ? ` · ${sideboardTotalCards} sideboard` : ""}` : totalCards ? `${totalCards > 100 ? "Over" : "Missing"} ${Math.abs(totalCards - 100)}${sideboardTotalCards ? ` · ${sideboardTotalCards} sideboard` : ""}` : "No deck" },
        ...
        <div style={{ display: "grid", gap: 18 }}>
          ...
          <section className="da-dashboard-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 14 }}>
            {[
              ...
              ["Downloadable analysis", "Generate a clean PDF with summary, diagnosis, charts, combos, recommendations and cuts.", "📋", "analysis"],
      ...
      {tab === "deckview" && (
        <div style={{ background: bg.card, borderRadius: 16, padding: 18, border: `1px solid ${bg.cardBorder}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 14 }}>
            <div>
              <h2 style={{ color: bg.text, margin: 0 }}>🗂️ Deck view</h2>
          ...
          <div className="deck-column-scroll" style={{ display: "flex", gap: 12, overflowX: deckViewLayout === "scroll" ? "auto" : "visible", overflowY: "visible", padding: "4px 4px 18px", alignItems: "flex-start", flexWrap: deckViewLayout === "wrap" ? "wrap" : "nowrap", maxWidth: "100%", WebkitOverflowScrolling: "touch" }}>
            {getDeckViewGroups().map(group => (
              <div key={group.name} style={{ minWidth: 178, maxWidth: deckViewLayout === "wrap" ? 190 : 178, background: isLightTheme ? "rgba(255,255,255,0.88)" : "#071207", border: `1px solid ${bg.cardBorder}`, borderRadius: 12, padding: 9, flex: deckViewLayout === "wrap" ? "1 1 178px" : "0 0 178px", boxShadow: "0 0 18px rgba(34,197,94,0.05)" }}>
                ...
                <div style={{ position: "relative", minHeight: group.cards.length ? 232 + Math.max(0, Math.min(group.cards.length, deckViewLayout === "wrap" ? 12 : 18) - 1) * 42 : 40, paddingBottom: 10 }}>
                  {group.cards.slice(0, deckViewLayout === "wrap" ? 12 : 18).map((card, idx) => (
                    <div
      ...
      {tab === "extras" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 14 }}>
          <div style={{ background: bg.card, borderRadius: 16, padding: 22, border: `1px solid ${bg.cardBorder}` }}>
            ...
            <p style={{ color: "#94a3b8", margin: "6px 0 18px", lineHeight: 1.5 }}>
              Detection first uses related tokens/all_parts from Scryfall and, if unavailable, Oracle text. Token detection is loaded on demand so the main deck analysis stays fast.
            </p>
            {!tokensLoaded && (
              <button onClick={() => setTokensLoaded(true)} style={{ background: "#14532d", color: "#86efac", border: "1px solid #22c55e", borderRadius: 10, padding: "10px 14px", cursor: "pointer", fontWeight: 900, marginBottom: 14 }}>
                Load token data
              </button>
            )}
            ...
            {!tokensLoaded && (
              <div style={{ color: "#94a3b8", background: "#0a150a", borderRadius: 10, padding: 14, marginBottom: 12 }}>Tokens are not loaded yet. Click Load token data to scan token sources and fetch token images.</div>
            )}
            {tokensLoaded && tokensAndExtras.tokens.length === 0 && tokensAndExtras.extras.length === 0 && (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 12 }}>
              {tokensAndExtras.tokens.map(token => (
                <div
                  key={token.name}
                  onMouseEnter={(e) => { if (token.image || token.smallImage || tokenImageData[normalizeName(token.name)]?.image || tokenImageData[normalizeName(token.name)]?.smallImage) { setPreviewCard({ name: token.name, image: token.image || token.smallImage || tokenImageData[normalizeName(token.name)]?.image || tokenImageData[normalizeName(token.name)]?.smallImage || tokenImageData[normalizeName(token.name)]?.image || tokenImageData[normalizeName(token.name)]?.smallImage, qty: 1, typeLine: token.typeLine || tokenImageData[normalizeName(token.name)]?.typeLine || "Token" }); setPreviewPos({ x: e.clientX, y: e.clientY }); } }}
                  onMouseMove={(e) => { if (token.image || token.smallImage || tokenImageData[normalizeName(token.name)]?.image || tokenImageData[normalizeName(token.name)]?.smallImage) setPreviewPos({ x: e.clientX, y: e.clientY }); }}
                  onMouseLeave={() => setPreviewCard(null)}
                  style={{ background: "#071207", border: "1px solid #14532d", borderRadius: 12, padding: 14, cursor: token.image || token.smallImage || tokenImageData[normalizeName(token.name)]?.image || tokenImageData[normalizeName(token.name)]?.smallImage ? "zoom-in" : "default" }}
                >
                  <div style={{ height: 230, borderRadius: 10, background: "radial-gradient(circle at 50% 25%, #14532d 0%, #020802 70%)", display: "flex", alignItems: "center", justifyContent: "center", color: "#86efac", fontSize: 22, fontWeight: 900, textAlign: "center", padding: 8, overflow: "hidden" }}>
                    {token.image || token.smallImage || tokenImageData[normalizeName(token.name)]?.image || tokenImageData[normalizeName(token.name)]?.smallImage ? (
                      <img src={token.image || token.smallImage || tokenImageData[normalizeName(token.name)]?.image || tokenImageData[normalizeName(token.name)]?.smallImage} alt={token.name} style={{ width: "100%", height: "100%", objectFit: "contain", borderRadius: 8 }} />
                    ) : tokenImageData[normalizeName(token.name)]?.notFound ? `${token.name}\nNo token image found` : `${token.name}\nLoading token image…`}
                  </div>
                  <div style={{ color: "#fff", fontWeight: 900, marginTop: 10 }}>{token.name}</div>
                  <div style={{ color: "#6b7280", fontSize: 11, marginTop: 2 }}>{tokenImageData[normalizeName(token.name)]?.typeLine || token.typeLine}</div>
                  <div style={{ color: "#94a3b8", fontSize: 12, marginTop: 8 }}>Generated by:</div>
        ...
        <div>
          <div style={{ background: bg.card, borderRadius: 12, padding: 16, border: `1px solid ${bg.cardBorder}`, marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <input
                ...
                style={{ background: bg.input, color: bg.text, border: `1px solid ${bg.cardBorder}`, borderRadius: 8, padding: "8px 10px", outline: "none", minWidth: 180 }}
              />
              <select
                value={typeFilter}
                onChange={e => setTypeFilter(e.target.value)}
                style={{ background: bg.input, color: bg.text, border: `1px solid ${bg.cardBorder}`, borderRadius: 8, padding: "8px 10px", outline: "none" }}
              >
              ...
              <select value={colorFilter} onChange={e => setColorFilter(e.target.value)} style={{ background: bg.input, color: bg.text, border: `1px solid ${bg.cardBorder}`, borderRadius: 8, padding: "8px 10px", outline: "none" }}>
                {allColorFilters.map(c => <option key={c} value={c}>Color: {c}</option>)}
              </select>
              <select value={cmcFilter} onChange={e => setCmcFilter(e.target.value)} style={{ background: bg.input, color: bg.text, border: `1px solid ${bg.cardBorder}`, borderRadius: 8, padding: "8px 10px", outline: "none" }}>
                {allCmcBuckets.map(c => <option key={c} value={c}>MV: {c}</option>)}
              </select>
              <select value={boardFilter} onChange={e => setBoardFilter(e.target.value)} style={{ background: bg.input, color: bg.text, border: `1px solid ${bg.cardBorder}`, borderRadius: 8, padding: "8px 10px", outline: "none" }}>
                {allBoardFilters.map(b => <option key={b} value={b}>Zone: {b}</option>)}
              </select>
              <select value={priceFilter} onChange={e => setPriceFilter(e.target.value)} style={{ background: bg.input, color: bg.text, border: `1px solid ${bg.cardBorder}`, borderRadius: 8, padding: "8px 10px", outline: "none" }}>
                {['All', 'Priced', 'No price', 'Budget <= 1', '1 - 5', '5 - 10', 'Premium > 10'].map(p => <option key={p} value={p}>Price: {p}</option>)}
              </select>
              <select value={cardSortBy} onChange={e => setCardSortBy(e.target.value)} style={{ background: bg.input, color: bg.text, border: `1px solid ${bg.cardBorder}`, borderRadius: 8, padding: "8px 10px", outline: "none" }}>
              ...
              {(activeFilter || cardSearch || typeFilter !== "All" || roleFilter !== "All" || colorFilter !== "All" || cmcFilter !== "All" || boardFilter !== "All" || priceFilter !== "All" || cardSortBy !== "name") && (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(170px, 1fr))", gap: 14 }}>
            ...
            >
              ...
              <div style={{ marginTop: 8, color: bg.text, fontWeight: 800, fontSize: 13 }}>{c.qty}x {c.name}</div>
              <div style={{ color: isLightTheme ? "#475569" : "#94a3b8", fontSize: 11 }}>{c.typeLine || c.type}</div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginTop: 5 }}>
                <span style={{ color: "#f59e0b", fontSize: 12, fontWeight: 950 }}>{Number(c.price) > 0 ? formatMoney(Number(c.price) * (c.qty || 1)) : "No price"}</span>
                {getCardBoard(c) !== "main" && <span style={{ color: "#38bdf8", fontSize: 10, fontWeight: 900 }}>{boardDisplayName(getCardBoard(c))}</span>}
              </div>
