...
function detectArchetypes(cards) {
  ...
  const negativeCounterQty = textCount([
    ...
    "put a -1/-1", "for each -1/-1", "remove a -1/-1", "creature with a -1/-1"
  ]) + (commanderFlags.hapatra ? 5 : 0);
  ...
  pushTheme("-1/-1 Counters", negativeCounterQty * 10 + roleCount.proliferate * 3 + (commanderFlags.hapatra ? 28 : 0), evidenceFor(card => hasAny(card, ["-1/-1 counter", "wither", "persist", "infect"]) || commanderName && normalizeName(card.name) === commanderName), ["Detecta cartas que ponen o aprovechan contadores -1/-1."]);
  ...
  pushTheme("Aristocrats", roleCount.sac * 13 + roleCount.drain * 11 + deathMatterQty * 5 + roleCount.tokens * 2 + roleCount.recursion * 2 - negativeCounterQty * 6, roleEvidence(["Sac Outlet", "Drain", "Tokens", "Recursion"]), [negativeCounterQty >= 4 ? "Penalizado porque el paquete de -1/-1 counters parece más dominante que sacrificio puro." : "Busca sac outlets + drain + death triggers."]);
  ...
  let subThemes = themeScores.slice(1, 5).filter(t => t.score >= Math.max(28, confidence * 0.45)).map(t => t.name);
  ...
  const mainResult = {
    ...
    macroScores,
    whyNot,
    edhrecStatus: "Comparación EDHREC: pendiente de integración.",
  };
...
export default function DeckAnalysis() {
  ...
  function cardFromDisplayLabel(label) {
    const clean = String(label || "").replace(/^\s*\d+x\s+/i, "").replace(/^Combo:\s*/i, "").trim();
    return findCard(cards, clean) || suggestionCardData[normalizeName(clean)] || null;
  }

  function showCardPreviewFromData(card, event) {
    if (!card) return;
    const data = card.image || card.smallImage ? card : suggestionCardData[normalizeName(card.name || "")];
    if (!data?.image && !data?.smallImage) return;
    setPreviewCard({ ...data, image: data.image || data.smallImage, qty: data.qty || 1 });
  ...
  async function ensureSuggestionCardLoaded(name) {
    ...
    try {
      ...
      setSuggestionCardData(prev => ({ ...prev, [key]: { name, image: "", smallImage: "", typeLine: "", colorIdentity: null, notFound: true } }));
    }
  }
...
Mycoloth`}
      ...
      <div style={{ textAlign: "center", color: "#31503a", fontSize: 11, marginTop: 20, fontFamily: "monospace", lineHeight: 1.6 }}>
        DECKFORGE ANALYZER · DASHBOARD · UNIVERSAL · v2.0<br />
