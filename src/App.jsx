...
function getCardBoard(card) {
  ...
  const raw = normalizeName(`${explicit} ${tagText}`);
  if (raw.includes("sideboard") || raw.includes("side board") || raw === "sb" || raw.includes(" sb ")) return "sideboard";
  if (raw.includes("maybeboard") || raw.includes("maybe board") || raw.includes("considering") || raw.includes("nodeck") || raw.includes("no deck")) return "maybeboard";
  if (explicit === "sideboard" || explicit === "maybeboard") return explicit;
  return "main";
}
...
async function fetchScryfallTokenImageCached(tokenName) {
  ...
  const queries = [
    `!\"${clean} Token\" include:extras type:token`,
    `\"${clean}\" include:extras type:token`,
    `${clean} include:extras type:token`,
  ];
  for (const query of queries) {
    try {
      const res = await fetch(`https://api.scryfall.com/cards/search?unique=cards&order=released&q=${encodeURIComponent(query)}`);
      if (!res.ok) continue;
      const data = await res.json();
      const hit = (data?.data || []).find(card => {
        const name = normalizeName(card.name || "");
        const type = normalizeName(card.type_line || "");
        return type.includes("token") && (name.includes(key) || key.includes(name.replace(" token", "")));
      }) || data?.data?.[0];
      if (!hit) continue;
...
function detectArchetypes(cards) {
  ...
  themeScores.forEach((theme, index) => {
    ...
    const ratio = raw / topRawThemeScore;
    // Relative score: prevents every synergistic subtheme from showing as 100.
    // 100 should mean "dominant theme", not merely "many cards also match this role".
    const normalized = Math.round(22 + ratio * 72 - index * 3.5);
    theme.score = Math.max(12, Math.min(index === 0 ? 96 : 91, normalized));
  });
...
export default function DeckAnalysis() {
  ...
  const tokensAndExtras = useMemo(() => detectTokensAndExtras(), [analysisCards]);
  ...
  useEffect(() => {
    ...
    if (tokensAndExtras.tokens?.length) loadTokenImages();
    return () => { cancelled = true; };
  }, [tokensAndExtras.tokens]);
  ...
  async function analyzeDeck() {
    ...
    await refreshSpellbookCombos(getAnalysisCards(enriched));
  }
  ...
  function makeReportChart(kind, title, data, options = {}) {
    ...
    const canvas = document.createElement("canvas");
    canvas.width = options.width || 760;
    canvas.height = options.height || 360;
    const ctx = canvas.getContext("2d");
    const W = canvas.width;
    const H = canvas.height;
    const palette = ["#16a34a", "#22c55e", "#0ea5e9", "#f59e0b", "#a855f7", "#ef4444", "#14b8a6", "#84cc16", "#64748b"];
    ctx.fillStyle = "#ffffff";
    ...
    const rows = (data || []).filter(x => Number(x.value ?? x.count) > 0).slice(0, options.limit || 9);
    if (!rows.length) {
      ctx.fillStyle = "#64748b";
      ctx.font = "600 18px Inter, Arial";
      ctx.fillText("No data available", 28, 90);
    ...
    if (kind === "donut") {
      ...
      rows.forEach((row, i) => {
        const y0 = 92 + i * 28;
        ctx.fillStyle = palette[i % palette.length];
        ctx.fillRect(360, y0 - 12, 16, 16);
        ctx.fillStyle = "#0f172a";
        ctx.font = "800 15px Inter, Arial";
        ctx.fillText(String(row.label || row.name || row.role || row.subject).slice(0, 28), 386, y0);
        ctx.fillStyle = "#475569";
        ctx.font = "700 14px Inter, Arial";
        ctx.fillText(String(Math.round(Number(row.value ?? row.count ?? 0))), 660, y0);
      });
      return canvas.toDataURL("image/png");
    }
    if (kind === "scatter") {
      ...
      const x0 = 78, y0 = H - 58, plotW = W - 128, plotH = H - 120;
      ...
      ctx.fillText("Mana value", x0 + plotW - 90, y0 + 28);
      ctx.save();
      ctx.translate(24, y0 - plotH / 2 + 40);
    ...
    rows.forEach((row, i) => {
      const y0 = 78 + i * 30;
      const label = String(row.label || row.name || row.subject || row.role || "Data").slice(0, 26);
      const value = Number(row.value ?? row.count ?? 0);
      ctx.fillStyle = "#0f172a";
      ctx.font = "800 14px Inter, Arial";
      ctx.fillText(label, 28, y0);
      ctx.fillStyle = "#dcfce7";
      ctx.fillRect(235, y0 - 14, 410, 16);
      ctx.fillStyle = palette[i % palette.length];
      ctx.fillRect(235, y0 - 14, Math.max(5, (value / max) * 410), 16);
      ctx.fillStyle = "#0f172a";
      ctx.font = "900 13px Inter, Arial";
      ctx.fillText(String(Math.round(value)), 660, y0);
    });
    return canvas.toDataURL("image/png");
  }
  ...
  async function downloadRichAnalysisPdf() {
    ...
    const newPage = () => { addFooter(); doc.addPage(); y = 44; };
    const need = (height) => { if (y + height > pageH - 52) newPage(); };
    const textBlock = (txt, x, maxW, size = 10, color = [15, 23, 42], bold = false, lineGap = 1.25) => {
      doc.setFont("helvetica", bold ? "bold" : "normal");
      doc.setFontSize(size);
      doc.setTextColor(...color);
      const lines = doc.splitTextToSize(pdfCleanText(txt), maxW);
      doc.text(lines, x, y);
      y += lines.length * size * lineGap + 6;
    };
    ...
    const chip = (label, x, yy, fill = [220, 252, 231], text = [22, 101, 52]) => {
      doc.setFillColor(...fill);
      doc.roundedRect(x, yy - 13, Math.min(190, 18 + String(label).length * 5.6), 20, 9, 9, "F");
      doc.setFont("helvetica", "bold");
      doc.setFontSize(8);
      doc.setTextColor(...text);
      doc.text(pdfCleanText(label), x + 9, yy);
    };

    doc.setFillColor(248, 250, 252);
    doc.rect(0, 0, pageW, pageH, "F");
    doc.setFillColor(236, 253, 245);
    doc.roundedRect(M, y, pageW - M * 2, 146, 18, 18, "F");
    doc.setDrawColor(187, 247, 208);
    doc.roundedRect(M, y, pageW - M * 2, 146, 18, 18, "S");
    const commanderData = await loadImageDataUrl(commander?.image || commander?.smallImage || "");
    if (commanderData) {
      doc.addImage(commanderData, "JPEG", M + 18, y + 18, 76, 106);
    } else {
      doc.setFillColor(20, 83, 45);
      doc.roundedRect(M + 18, y + 18, 76, 106, 8, 8, "F");
      doc.setTextColor(220, 252, 231);
      doc.setFontSize(10);
      doc.text("Commander", M + 26, y + 72);
    }
    doc.setFont("helvetica", "bold");
    doc.setFontSize(12);
    doc.setTextColor(22, 163, 74);
    doc.text("COMMANDER DECK ANALYSIS", M + 112, y + 34);
    doc.setFontSize(24);
    doc.setTextColor(15, 23, 42);
    doc.text(pdfCleanText(commander?.name || deckName || "Commander not detected"), M + 112, y + 66, { maxWidth: 310 });
    chip(`Overall ${scores.overall}/10`, M + 112, y + 100, [254, 243, 199], [146, 64, 14]);
    chip(`Bracket ${bracket.bracket} - ${bracket.shortLabel}`, M + 222, y + 100);
    chip(`${formatMoney(totalDeckPrice)}`, M + 370, y + 100, [219, 234, 254], [30, 64, 175]);
    y += 170;
    ...
    summary.forEach(item => textBlock(`- ${item}`, M + 8, pageW - M * 2 - 16, 10));

    const chartW = 238;
    const chartH = 150;
    need(330);
    const diagnosisChart = makeReportChart("bar", "Role diagnosis", scores.radar.map(x => ({ label: x.subject, value: x.value })), { width: 760, height: 360, limit: 8 });
    const roleDonut = makeReportChart("donut", "Role mix", roles.slice(0, 8).map(x => ({ label: x.role, value: x.count })), { width: 760, height: 360 });
    if (diagnosisChart) doc.addImage(diagnosisChart, "PNG", M, y, chartW, chartH);
    if (roleDonut) doc.addImage(roleDonut, "PNG", M + chartW + 26, y, chartW, chartH);
    y += chartH + 18;
    const curveChart = makeReportChart("bar", "Mana curve", manaCurve.map(x => ({ label: `MV ${x.cmc}`, value: x.count })), { width: 760, height: 300, limit: 8 });
    const scatterChart = makeReportChart("scatter", "Price vs mana value", analysisCards.filter(c => Number(c.price) > 0 && typeof c.cmc === "number").sort((a, b) => Number(b.price) - Number(a.price)).slice(0, 24).map(c => ({ label: c.name, x: c.cmc || 0, y: Number(c.price) || 0 })), { width: 760, height: 300 });
    if (curveChart) doc.addImage(curveChart, "PNG", M, y, chartW, chartH);
    if (scatterChart) doc.addImage(scatterChart, "PNG", M + chartW + 26, y, chartW, chartH);
    y += chartH + 12;

    sectionTitle("Archetype explained");
    textBlock(`Macroarchetype: ${archetypes[0]?.macroArchetype || "Not detected"}`, M, pageW - M * 2, 11, [15, 23, 42], true);
    ...
    (archetypes[0]?.explanation || []).slice(0, 5).forEach(line => textBlock(`- ${line}`, M + 8, pageW - M * 2 - 16, 9));

    sectionTitle("Strengths & weaknesses");
    textBlock("Strengths", M, pageW - M * 2, 12, [22, 101, 52], true);
    strengths.slice(0, 5).forEach(x => textBlock(`- ${x.title}: ${x.text}`, M + 8, pageW - M * 2 - 16, 9));
    textBlock("Weaknesses", M, pageW - M * 2, 12, [153, 27, 27], true);
    weaknesses.slice(0, 5).forEach(x => textBlock(`- ${x.title}: ${x.text}`, M + 8, pageW - M * 2 - 16, 9));

    sectionTitle("Combos, upgrades and cuts");
    textBlock("Included combos", M, pageW - M * 2, 12, [22, 101, 52], true);
    (includedCombos.length ? includedCombos.slice(0, 8) : [{ name: "No complete combos detected", effects: [] }]).forEach(x => textBlock(`- ${x.name}${x.effects?.length ? `: ${x.effects.join(" / ")}` : ""}`, M + 8, pageW - M * 2 - 16, 9));
    textBlock("Almost included", M, pageW - M * 2, 12, [146, 64, 14], true);
    almostCombos.slice(0, 8).forEach(x => textBlock(`- ${x.name}: missing ${(x.missing || []).join(", ")}`, M + 8, pageW - M * 2 - 16, 9));
    textBlock("Recommended upgrades", M, pageW - M * 2, 12, [30, 64, 175], true);
    recommendations.slice(0, 8).forEach(x => textBlock(`- ${x.area}: ${x.card} (${x.priority})`, M + 8, pageW - M * 2 - 16, 9));
    textBlock("Possible cuts", M, pageW - M * 2, 12, [153, 27, 27], true);
    cutCandidates.slice(0, 8).forEach(x => textBlock(`- ${x.card.name}: ${(x.reasons || []).slice(0, 2).join(" / ") || "reviewable slot"}`, M + 8, pageW - M * 2 - 16, 9));

    sectionTitle("Showcase summary");
    textBlock("This report intentionally omits the full decklist so it stays clean and easy to share. Use the app's Deck and Cards tabs for the full list, prices and alternate art choices.", M, pageW - M * 2, 10, [71, 85, 105]);
