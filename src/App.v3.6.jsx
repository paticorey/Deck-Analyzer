...
export default function DeckAnalysis() {
  ...
  function makeReportChart(kind, title, data, options = {}) {
    ...
    if (ctx.roundRect) ctx.stroke(); else ctx.strokeRect(12, 12, W - 24, H - 24);
    ...
    rows.forEach((row, i) => {
      ...
      if (ctx.roundRect) ctx.fill(); else ctx.fillRect(left + labelW, y0, barW, 18);
      ctx.fillStyle = palette[i % palette.length];
      const filled = Math.max(4, (value / max) * barW);
      ctx.roundRect?.(left + labelW, y0, filled, 18, 9);
      if (ctx.roundRect) ctx.fill(); else ctx.fillRect(left + labelW, y0, filled, 18);
      ctx.fillStyle = "#0f172a";
  ...
  async function downloadRichAnalysisPdf() {
    ...
    doc.roundedRect(M, y, pageW - M * 2, 160, 18, 18, "F");
    doc.setDrawColor(...color.line);
    doc.roundedRect(M, y, pageW - M * 2, 160, 18, 18, "S");
    const commanderData = await loadImageDataUrl(commander?.image || commander?.smallImage || "");
    if (commanderData && safeAddImage(commanderData, M + 18, y + 16, 92, 128)) {
      doc.setDrawColor(20, 83, 45);
      doc.roundedRect(M + 18, y + 16, 92, 128, 7, 7, "S");
    } else {
      doc.setFillColor(...color.green);
      doc.roundedRect(M + 18, y + 16, 92, 128, 8, 8, "F");
      doc.setFont("helvetica", "bold");
      doc.setFontSize(11);
      doc.setTextColor(220, 252, 231);
      doc.text("Commander", M + 31, y + 84);
    }
    const titleX = M + 128;
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    setText(color.bright);
    doc.text("COMMANDER DECK ANALYSIS", titleX, y + 32);
    const afterNameY = fixedText(commander?.name || deckName || "Commander not detected", titleX, y + 62, pageW - titleX - M - 10, 25, color.ink, true, 1.05);
    const chipY = Math.min(y + 132, Math.max(y + 100, afterNameY + 18));
    let chipX = titleX;
    ...
    y += 6;
    addChartImage(roleChart, M, y, pageW - M * 2, 235);
    y += 250;
    addFooter();
    ...
    addChartImage(roleDonut, M, y, halfW, 220);
    addChartImage(curveChart, M + halfW + 16, y, halfW, 220);
    y += 238;
    addChartImage(topValueChart, M, y, pageW - M * 2, 250);
    y += 270;
    textBlock("Top value cards are ordered by estimated total price. Role, curve and price charts use the main deck only; sideboard and maybeboard are excluded.", M, pageW - M * 2, 9.5, color.muted);
    addFooter();
