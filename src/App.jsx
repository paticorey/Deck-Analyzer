...
const ROLE_DESCRIPTIONS = {
  ...
  "-1/-1 Counters": "Pone o aprovecha contadores -1/-1, blight, wither, infect, persist o proliferate sobre esos contadores.",
...
const COMMANDER_THEME_HINTS = {
  "high perfect morcant": {
    ...
    reason: "El comandante blightea y prolifera; eso convierte los contadores -1/-1 en el eje real del mazo, no solo en soporte."
  },
  "hapatra, vizier of poisons": {
    ...
    reason: "Hapatra recompensa directamente poner contadores -1/-1 creando tokens; tokens/drain suelen ser subplanes."
  },
  "dina, soul steeper": {
    ...
    reason: "Dina transforma lifegain en drenaje; sacrificio puede aparecer, pero no siempre es el plan principal."
  },
  "giada, font of hope": {
    ...
    reason: "Giada escala ángeles y empuja un plan typal de criaturas grandes con counters."
  },
  "selvala, heart of the wilds": {
    ...
    reason: "Selvala convierte criaturas grandes en maná y cartas, típico de Big Mana con techo combo."
  },
  "yuriko, the tiger's shadow": {
    ...
    reason: "Yuriko empuja ninjas/evasión y manipulación de top como plan central."
  },
  "muldrotha, the gravetide": {
    ...
    reason: "Muldrotha usa el cementerio como segunda mano, por encima de recursion incidental."
  }
};
...
function detectArchetypes(cards) {
  ...
  const creatures = typeCount("Criaturas");
  const instants = typeCount("Instantáneos");
  const sorceries = typeCount("Sortilegios");
  const artifacts = typeCount("Artefactos");
  const enchantments = typeCount("Encantamientos");
  const commander = getCommander(cards);
  ...
  const totalNonLands = cards.filter(c => c.type !== "Tierras" && c.type !== "Commander").reduce((a, c) => a + c.qty, 0) || 1;
  ...
  pushTheme("-1/-1 Counters", negativeCounterQty * 12 + roleCount.negativeCounters * 8 + roleCount.proliferate * 5 + (commanderFlags.hapatra ? 34 : 0) + (commanderFlags.morcant ? 42 : 0) + (commanderHint?.mainTheme === "-1/-1 Counters" ? commanderHint.weight || 0 : 0), evidenceFor(card => hasAny(card, ["-1/-1 counter", "blight", "blights", "wither", "persist", "infect", "proliferate"]) || (commanderName && normalizeName(card.name) === commanderName)), ["Detecta cartas que ponen o aprovechan contadores -1/-1, incluyendo blight/wither/infect/persist y proliferate."]);
  pushTheme("+1/+1 Counters", plusCounterQty * 8 + roleCount.proliferate * 7, roleEvidence(["Counters", "Proliferate"]), ["Cuenta payoffs de contadores positivos y proliferate."]);
  pushTheme("Aristocrats", roleCount.sac * 13 + roleCount.drain * 10 + deathMatterQty * 5 + roleCount.tokens * 2 + roleCount.recursion * 2 - negativeCounterQty * 10 - (commanderHint?.mainTheme === "-1/-1 Counters" ? 18 : 0), roleEvidence(["Sac Outlet", "Drain", "Tokens", "Recursion"]), [negativeCounterQty >= 4 ? "Penalizado porque el paquete de -1/-1 counters/blight parece más dominante que sacrificio puro." : "Busca sac outlets + drain + death triggers."]);
  pushTheme("Lifegain / Drain", roleCount.lifegain * 9 + roleCount.drain * 10 + (commanderFlags.dina ? 22 : 0), roleEvidence(["Lifegain", "Drain"]), ["Diferencia lifegain/drain de Aristocrats cuando no depende de sacrificar criaturas."]);
  pushTheme("Tokens", roleCount.tokens * 8 + (roleCount.finisher >= 2 ? 6 : 0) + (roleCount.sac >= 2 ? 4 : 0), roleEvidence(["Tokens", "Finisher", "Sac Outlet"]), ["Detecta producción de tokens y cartas que los convierten en presión o recursos."]);
  pushTheme("Spellslinger", roleCount.spellslinger * 18 + (spells >= 24 ? 24 : spells >= 18 ? 12 : 0), roleEvidence(["Spellslinger", "Draw", "Counterspell"]), [`${spells} instantáneos/conjuros detectados.`]);
  pushTheme("Voltron", roleCount.voltron * 14 + roleCount.equipment * 9 + count("Evasion") * 4, roleEvidence(["Voltron", "Equipment", "Evasion", "Protection"]), ["Busca plan de comandante/criatura grande protegida y evasiva."]);
  pushTheme("Reanimator", roleCount.reanimator * 13 + roleCount.recursion * 6 + textCount(["return target creature card from your graveyard to the battlefield", "reanimate", "from your graveyard to the battlefield"]) * 8, roleEvidence(["Reanimator", "Recursion"]), ["Reanimar directo pesa más que simple recursion a mano."]);
  pushTheme("Blink / ETB", roleCount.blink * 15 + roleCount.etb * 6 + (commanderFlags.brago ? 22 : 0), roleEvidence(["Blink", "ETB"]), ["Busca repetir entradas al campo o exiliar/devolver permanentes."]);
  pushTheme("Landfall", landfallQty * 8 + (roleCount.ramp >= 9 ? 8 : 0), evidenceFor(card => hasAny(card, ["landfall", "whenever a land enters", "play an additional land", "from your graveyard"])), ["Cuenta triggers de tierra, land recursion y jugar tierras extra."]);
  pushTheme("Artifacts", roleCount.artifactRole * 15 + Math.max(0, artifacts - roleCount.ramp) * 2, roleEvidence(["Artifacts", "Combo", "Ramp"]), ["No cuenta automáticamente todos los mana rocks como tema artifacts."]);
  pushTheme("Enchantress", roleCount.enchantress * 16 + enchantments * 2, roleEvidence(["Enchantress"]), ["Busca encantamientos, auras, constellation o payoffs tipo enchantress."]);
  pushTheme("Tribal / Typal", roleCount.tribal * 16 + (creatures >= 32 ? 11 : 0) + (commanderFlags.giada || commanderFlags.yuriko ? 22 : 0), roleEvidence(["Tribal"]), ["Mira lord effects, tipos de criatura y densidad tribal."]);
  pushTheme("Stax", roleCount.stax * 17 + textCount(["players can't", "opponents can't", "doesn't untap", "skip", "tax", "unless they pay"]) * 4, roleEvidence(["Stax"]), ["Identifica impuestos, locks y limitaciones de acciones."]);
  pushTheme("Group Hug", groupHugQty * 7, evidenceFor(card => hasAny(card, ["each player draws", "each player may", "each opponent may"])), ["Detecta efectos simétricos o políticos que ayudan a la mesa."]);
  pushTheme("Mill", millQty * 9, evidenceFor(card => hasAny(card, ["mill", "mills"])), ["Detecta mill como plan real, no solo autollenado incidental."]);
  pushTheme("Graveyard Value", roleCount.recursion * 10 + (commanderFlags.muldrotha ? 35 : 0) + textCount(["you may play", "from your graveyard", "escape", "flashback"]) * 3, roleEvidence(["Recursion", "Reanimator", "Sac Outlet"]), ["Cementerio como segunda mano o motor de value."]);
  pushTheme("Big Mana", bigManaQty * 5 + roleCount.finisher * 5, roleEvidence(["Ramp", "Untap", "Finisher"]), ["Ramp alto, untap y costes grandes empujan a Big Mana."]);
  pushTheme("Creature Combo", compactCombos.length * 30 + roleCount.untap * 9 + roleCount.tutor * 5 + (commanderFlags.selvala ? 26 : 0), [...new Set([...compactCombos.flatMap(c => c.pieces), ...roleEvidence(["Untap", "Tutor", "Combo"])])], [compactCombos.length ? `${compactCombos.length} combo(s) compacto(s) detectado(s).` : "Busca loops compactos y piezas de untap/tutor."]);
  pushTheme("Sacrifice", roleCount.sac * 13 + deathMatterQty * 5, roleEvidence(["Sac Outlet", "Recursion", "Tokens"]), ["Sacrificio como motor, aunque no necesariamente Aristocrats."]);
  ...
  if (mainTheme === "-1/-1 Counters") {
    explanation.push("Los payoffs de tokens/drain se tratan como subplanes si dependen de los contadores, no como Aristocrats por defecto.");
    explanation.push("El mazo gana por acumulación de engines, triggers y posibles loops; no por presión aggro pura salvo que el resto de cartas lo indique.");
  } else if (roleCount.tokens >= 3 && roleCount.drain >= 2 && mainTheme !== "Aristocrats") {
    explanation.push("Tokens y drain aparecen como soporte, pero no superan al tema principal detectado.");
  } else if (completedCombos.length) {
    explanation.push(`Hay ${completedCombos.length} combo(s) completo(s), por eso Combo puede aparecer como macroplan o subplan.`);
  } else if (topMacro.name === "Midrange") {
    explanation.push("La estructura parece orientada a value incremental, interacción y motores de mesa.");
  }
  explanation.push(`La confianza sale de la diferencia entre ${mainTheme} (${confidence}) y los siguientes temas detectados.`);

  const whyNot = themeScores.slice(1, 6).map(t => {
    if (t.name === "Aristocrats" && mainTheme === "-1/-1 Counters") return `No se elige Aristocrats porque ${mainTheme} tiene más densidad específica; sac/drain quedan como soporte.`;
    return `No se elige ${t.name} como principal porque su puntuación (${t.score}) queda por debajo de ${mainTheme} (${confidence}).`;
  });
  ...
  const mainResult = {
    ...
    macroScores,
    whyNot,
    edhrecStatus: "Comparación EDHREC: pendiente de integración. No se inventan datos; cuando haya integración real se usará como contraste, no como fuente única.",
  };
...
export default function DeckAnalysis() {
  ...
  function generateReportText() {
    ...
    const topCuts = cutCandidates.slice(0, 6).map(x => `- ${x.card.name}: ${(x.reasons || []).slice(0, 2).join(" · ") || "slot revisable"}`).join(String.fromCharCode(10));
    const completeCombos = includedCombos.slice(0, 6).map(x => `- ${x.name}`).join(String.fromCharCode(10)) || "- No se detectaron combos completos.";
    return [
      `DeckForge Analyzer — Reporte compartible`,
      `Commander: ${commander?.name || "No detectado"}`,
      `Overall: ${scores.overall}/10`,
      `Bracket: ${bracket.bracket} (${bracket.shortLabel})`,
      `Precio estimado: ${formatMoney(totalDeckPrice)} · ${unpricedCardsCount} cartas sin precio detectado`,
      `Macroarquetipo: ${archetype.macroArchetype || "No detectado"}`,
      `Tema principal: ${archetype.mainTheme || "No detectado"}`,
      `Subplanes: ${(archetype.subThemes || []).join(" / ") || "No detectados"}`,
      `Confianza arquetipo: ${archetype.confidence || archetype.score || "?"}%`,
      "",
      "Plan de juego:",
      planText,
      "",
      ...
      "",
      "Cortes sugeridos:",
      topCuts || "- Sin cortes claros detectados.",
      "",
  ...
  async function ensureSuggestionCardLoaded(name) {
    ...
    if (!key || suggestionCardData[key]) return;
    try {
      const data = await fetchScryfallCard(clean);
      const scry = scryToCardData(data);
      ...
      setSuggestionCardData(prev => ({ ...prev, [key]: { name: clean, image: "", smallImage: "", typeLine: "", colorIdentity: null, notFound: true } }));
    }
  }
  ...
  const statCards = [
    ...
    { label: "Precio", value: cards.length ? formatMoney(totalDeckPrice) : "-", icon: "💶", color: "#facc15", sub: cards.length ? `${unpricedCardsCount} cartas sin precio` : "cargar Scryfall" },
    { label: "Arquetipo", value: archetypes[0]?.mainTheme || "-", icon: "🎭", color: "#a855f7", sub: archetypes[0]?.macroArchetype || "no detectado" },
        ...
        <div className="da-topbar" style={{ background: bg.nav, border: `1px solid ${bg.cardBorder}`, borderRadius: 18, boxShadow: bg.shadow, padding: 10, marginBottom: 16 }}>
          <div className="da-topbar-inner" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <button onClick={resetAnalyzer} className="da-soft-button" style={{ background: "#16a34a", color: "#fff", border: "1px solid rgba(255,255,255,.2)", borderRadius: 999, padding: "8px 13px", cursor: "pointer", fontWeight: 900 }}>+ Nuevo análisis limpio</button>
      ...
      {tab === "home" && (
        <div style={{ display: "grid", gap: 18 }}>
                    ...
                    <div style={{ padding: 14 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                        <div style={{ background: isLightTheme ? "#f8fafc" : "#061006", borderRadius: 9, padding: 8 }}>
                          ...
                          <div style={{ color: bg.muted, fontSize: 9, textTransform: "uppercase" }}>Precio</div>
                        </div>
                        <div style={{ background: isLightTheme ? "#f8fafc" : "#061006", borderRadius: 9, padding: 8 }}>
                          <div style={{ color: "#a78bfa", fontWeight: 950, fontSize: 13 }}>{deck.summary?.archetype?.confidence || "?"}%</div>
                          <div style={{ color: bg.muted, fontSize: 9, textTransform: "uppercase" }}>Confianza</div>
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        <Tag label={deck.summary?.archetype?.mainTheme || deck.summary?.archetypes?.[0] || "Arquetipo"} />
          ...
          <section className="da-dashboard-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 14 }}>
            {[
              ["Bracket explicado", "Detecta Game Changers, combos compactos, tutores, fast mana y locks para estimar experiencia de mesa.", "🏷️", "bracket"],
              ["Combos detectados", "Separá combos incluidos y casi incluidos, con filtros por tamaño y cartas faltantes reales.", "🧩", "combos"],
              ["Mejoras accionables", "Te sugiere cartas a meter y cortes según roles flojos, arquetipo e identidad de color.", "🔧", "recommendations"],
              ["Reporte compartible", "Copiá un resumen con commander, overall, bracket, precio, arquetipo, combos y cortes.", "📋", "analysis"],
              ["Mis decks", "Abrí tus análisis guardados desde localStorage como una biblioteca visual.", "📚", "decks"],
              ["Precio estimado", "Revisá el precio total estimado y cuántas cartas quedaron sin precio.", "💶", "stats"],
              ...
              <button key={title} onClick={() => setTab(target)} className="da-card-hover" style={{ textAlign: "left", background: bg.card, border: `1px solid ${bg.cardBorder}`, borderRadius: 20, padding: 20, boxShadow: bg.shadow, cursor: "pointer", fontFamily: "inherit" }}>
                ...
                <div style={{ color: "#22c55e", fontSize: 12, marginTop: 12, fontWeight: 900 }}>Abrir →</div>
              </button>
            ))}
          </section>
        </div>
              ...
              <div key={deck.id} style={{ background: "linear-gradient(135deg, #081308 0%, #0d1a0d 100%)", borderRadius: 16, padding: 18, border: "1px solid #14532d", boxShadow: "0 0 28px rgba(34,197,94,0.06)" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginTop: 14 }}>
                  ...
                  <div style={{ background: "#061006", borderRadius: 10, padding: 10, textAlign: "center" }}>
                    ...
                    <div style={{ color: "#6b7280", fontSize: 10, textTransform: "uppercase" }}>Precio</div>
                  </div>
                </div>
          ...
          <div style={{ background: bg.card, borderRadius: 12, padding: 20, border: `1px solid ${bg.cardBorder}` }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
              ...
              <button onClick={() => setComboSizeFilter("4plus")} style={{ background: comboSizeFilter === "4plus" ? "#14532d" : "#071207", color: comboSizeFilter === "4plus" ? "#86efac" : "#94a3b8", border: "1px solid #14532d", borderRadius: 999, padding: "7px 12px", cursor: "pointer", fontWeight: 800 }}>
                4+ cartas ({fourPlusComboCount})
              </button>
            </div>
            {displayedCombos.length === 0 && (
              <div style={{ color: "#94a3b8", background: "#0a150a", borderRadius: 8, padding: 12 }}>
                {comboView === "included" ? "No hay combos completos detectados." : "No hay combos casi completos. Se muestran cuando te falta exactamente una pieza y respeta la identidad de color del comandante."}
              </div>
            )}
          ...
          <div style={{ background: bg.card, borderRadius: 16, padding: 22, border: "1px solid #a855f755", gridColumn: "1 / -1" }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start", flexWrap: "wrap" }}>
              ...
              <div style={{ background: "#071207", border: "1px solid #14532d", borderRadius: 12, padding: 12, minWidth: 240 }}>
                <div style={{ color: "#86efac", fontWeight: 900, marginBottom: 6 }}>Subplanes</div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{(archetypes[0]?.subThemes?.length ? archetypes[0].subThemes : ["Sin subplanes fuertes"]).map(x => <Tag key={x} label={x} />)}</div>
              </div>
            </div>
            <details style={{ marginTop: 16 }} open>
              <summary style={{ color: "#86efac", cursor: "pointer", fontWeight: 900 }}>¿Por qué este arquetipo?</summary>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 12, marginTop: 12 }}>
                <div style={{ background: "#071207", border: "1px solid #14532d", borderRadius: 12, padding: 14 }}>
                  <div style={{ color: "#86efac", fontWeight: 900, marginBottom: 8 }}>Explicación</div>
                ...
                <div style={{ background: "#071207", border: "1px solid #14532d", borderRadius: 12, padding: 14 }}>
                  <div style={{ color: "#86efac", fontWeight: 900, marginBottom: 8 }}>Puntuación por tema</div>
                ...
                <div style={{ background: "#071207", border: "1px solid #14532d", borderRadius: 12, padding: 14 }}>
                  <div style={{ color: "#86efac", fontWeight: 900, marginBottom: 8 }}>Cartas evidencia</div>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  ...
                  <div style={{ color: "#86efac", fontWeight: 900, margin: "14px 0 8px" }}>Por qué no otros</div>
      ...
      {tab === "cards" && (
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(170px, 1fr))", gap: 14 }}>
            {filteredCards.map(c => <div
              key={c.key}
              onMouseEnter={(e) => showCardPreviewFromData(c, e)}
              onMouseMove={(e) => showCardPreviewFromData(c, e)}
              onMouseLeave={() => setPreviewCard(null)}
              style={{ background: bg.card, borderRadius: 12, padding: 10, border: `1px solid ${bg.cardBorder}`, cursor: c.image || c.smallImage ? "zoom-in" : "default" }}
            >
              {c.image ? <img src={c.image} alt={c.name} style={{ width: "100%", borderRadius: 10, display: "block" }} /> : <div style={{ height: 230, borderRadius: 10, background: "#050d05", display: "flex", alignItems: "center", justifyContent: "center", color: "#6b7280", textAlign: "center" }}>Sin imagen<br />Cargá Scryfall</div>}
        ...
        >
          {previewCard.image ? (
            ...
            <div style={{ height: 438, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", textAlign: "center", padding: 18, background: "linear-gradient(135deg,#061006,#020802)", border: "1px solid #14532d", color: previewCard.notFound ? "#fbbf24" : "#86efac", fontWeight: 900 }}>
              {previewCard.loading ? "Cargando imagen…" : "Sin imagen disponible"}
            </div>
          )}
